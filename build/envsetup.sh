...
function breakfast()
{
    target=$1
    local variant=$2
    source ${ANDROID_BUILD_TOP}/vendor/superior/vars/aosp_target_release
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Superior model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch superior_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/superior-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.superior.device | grep -q "$SUPERIOR_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $SUPERIOR_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function lineageremote()
{
    ...
    git remote rm superior 2> /dev/null
    ...
    then
        ...
        local PFX="SuperiorOS/"
    else
        local PROJECT=$REMOTE
    fi

    local SUPERIOR_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$SUPERIOR_USER" ]
    then
        git remote add superior ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add superior ssh://$SUPERIOR_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'superior' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SuperiorOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.superior.device | grep -q "$SUPERIOR_BUILD");
    then
...
function lineagerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SuperiorOS Gerrit Rebase Usage: "
...
function dopush()
{
    ...
    if (adb shell getprop ro.superior.device | grep -q "$SUPERIOR_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SUPERIOR_FIXUP_COMMON_OUT ]; then
...
function build_kernel() {
    ...
    local superior_version="superior-$(_get_build_var_cached PRODUCT_VERSION_MAJOR).$(_get_build_var_cached PRODUCT_VERSION_MINOR)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${superior_version}")
        if [ -n "${SUPERIOR_MIRROR}" ]; then
            repo_init_args+=("--reference" "${SUPERIOR_MIRROR}")
        fi
